...
function ShopLogo() {
  return (
    <svg width="56" height="56" viewBox="0 0 56 56" fill="none" xmlns="http://www.w3.org/2000/svg">
      <rect x="0.4" y="0.4" width="55.2" height="55.2" rx="3.6" fill="white" stroke="#E6E6E6" strokeWidth="0.8" />
      <path
      ...
      <path
        ...
        fill="#009F06"
      />
      <text fill="#555555" style={{ whiteSpace: "pre" }} fontFamily="Dancing Script" fontSize="13" fontWeight="bold" letterSpacing="0em">
        <tspan x="8" y="39.66">farmary</tspan>
      </text>
    </svg>
  );
}
...
export default function ProductDetails({ product = mockProduct }) {
  const [selectedImageIndex, setSelectedImageIndex] = useState(0);
  const [quantity, setQuantity] = useState(5);
  const [isWishlisted, setIsWishlisted] = useState(false);

  const visibleStart = Math.max(0, Math.min(selectedImageIndex - 1, product.images.length - 4));
  const visibleImages = product.images.slice(visibleStart, visibleStart + 4);
      ...
      <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
          ...
          <div className="flex flex-col items-center gap-3 w-[80px] flex-shrink-0">
            ...
            >
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                <path d="M19 15.5L12 8.5L5 15.5" stroke="#999999" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </button>
        ...
        <div className="flex flex-col gap-5 lg:w-[50%]">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div className="flex items-center gap-1">
                ...
                <span className="text-[#666] text-sm ml-1">{product.reviewCount} lượt đánh giá</span>
              </div>
          ...
          <div className="flex items-center gap-3">
            <span className="text-[#B3B3B3] text-xl line-through">{formatPrice(product.originalPrice)}</span>
            <span className="text-[#2C742F] text-2xl font-medium">{formatPrice(product.salePrice)}</span>
            ...
            <div className="flex items-center gap-1 px-2 py-2 rounded-full border border-[#E6E6E6] bg-white">
              ...
              >
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                  <path d="M2.3335 7H11.6668" stroke="#666666" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </button>
              <span className="w-10 text-center text-[#1A1A1A] text-base">{quantity}</span>
              <button
                onClick={() => setQuantity((q) => q + 1)}
                className="w-[34px] h-[34px] rounded-full bg-[#F2F2F2] flex items-center justify-center hover:bg-gray-200 transition-colors"
              >
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                  <path d="M2.3335 7.00004H11.6668M7.00016 2.33337V11.6667" stroke="#1A1A1A" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </button>
            </div>
          ...
          <div className="flex flex-col gap-3">
            <div className="flex flex-wrap items-center gap-1.5">
              <span className="text-[#1A1A1A] text-sm font-medium">Danh mục:</span>
